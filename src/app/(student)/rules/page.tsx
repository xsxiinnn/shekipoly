...
import { getMissionImage, getTeamTheme } from "@/config/team-themes";
...
function SectionTitle({ eyebrow, children }: { eyebrow: string; children: React.ReactNode }) {
  return (
    <div className="text-team-on-primary">
      <p className="text-xs font-black tracking-[0.16em]">{eyebrow}</p>
...
export default async function RulesPage() {
  const [data, activityScope, profileResult] = await Promise.all([
    getRulesData(),
  ...
  return (
    <div className="min-w-0 overflow-x-clip pb-4 pt-2">
      <header className="text-team-on-primary">
        ...
        <p className="mt-2 text-base font-bold text-team-on-primary">一起走進他的世界</p>
      </header>
        ...
        {data.missions.length > 0 ? (
          <div className="mt-4 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2">
            {data.missions.map((mission) => {
              const missionImage = getMissionImage(teamTheme?.slug, mission.id);
              const missionCardName = MISSION_CARD_NAMES[mission.id - 1] ?? mission.name;

              return missionImage ? (
                <article
                  key={mission.id}
                  className="min-w-0 overflow-hidden rounded-2xl border border-border bg-white p-2 shadow-sm"
                >
                  <div className="relative aspect-[1240/1748] w-full overflow-hidden rounded-xl bg-brand-soft">
                    <Image
                      src={missionImage}
                      alt={`${missionCardName}任務圖卡；一般 ${mission.baseScore} 步，3×5 認領名單 ${mission.baseScore * 2} 步`}
                      fill
                      sizes="(max-width: 359px) calc(100vw - 48px), (max-width: 767px) calc((100vw - 60px) / 2), 190px"
                ...
                <article key={mission.id} className="rounded-3xl border border-border bg-white p-5 shadow-sm">
                  <h3 className="text-base font-black leading-6">{missionCardName}</h3>
      ...
      <section className="mt-9 rounded-3xl border border-[#d7c6f0] bg-[#f4edff] p-5" aria-labelledby="photo-bonus-title">
        ...
        <div className="mt-4 rounded-2xl bg-white/80 p-4 text-sm leading-7">
          <p className="font-black">3×5「邀約烤肉」</p>
        ...
        <div className="mt-4 space-y-3">
          <article className="rounded-3xl border border-border bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-muted">
              如果同一次和同一位朋友完成見面聊聊、祝福小物、為他禱告與邀約烤肉，只選步數最高的「邀約烤肉」，不要把所有任務加總。
            </p>
          </article>
          <article className="rounded-3xl border border-border bg-white p-5 shadow-sm">
      ...
      <section className="mt-9" aria-labelledby="dates-title">
        ...
        <p className="mt-3 text-xs font-semibold text-team-on-primary">活動時間以 Asia/Taipei（台北時間）為準。</p>
      </section>
    </div>
  );
}
