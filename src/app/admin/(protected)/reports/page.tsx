...
function DataBadge({ isTest }: { isTest: boolean }) {
  return <span className={`rounded-full px-2 py-1 text-xs font-black ${isTest ? "bg-violet-100 text-violet-800" : "bg-sky-50 text-sky-700"}`}>{isTest ? "TEST" : "OFFICIAL"}</span>;
}
      ...
      <div className="space-y-3 md:hidden">
        {data.items.map((report) => (
          <Link key={report.id} href={`/admin/reports/${report.id}`} className="block rounded-2xl border border-border bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted">W{report.activityWeek}・{formatAdminDateTime(report.createdAt)}</p><h2 className="mt-1 font-black">{report.teamName}</h2></div><div className="flex gap-1"><DataBadge isTest={report.isTest} /><StatusBadge status={report.status} /></div></div>
