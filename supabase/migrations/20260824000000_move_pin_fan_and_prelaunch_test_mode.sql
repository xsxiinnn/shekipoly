...
-- Move the one existing team record while preserving its UUID. The legacy
-- profiles.zone_id compatibility column is kept consistent in this transaction.
do $$
declare
  target_zone_id bigint;
  target_team_id uuid;
  matching_teams integer;
  changed_teams integer;
begin
  select count(*) into matching_teams
  from public.teams
  where name = '品凡小組';

  if matching_teams <> 1 then
    raise exception 'Expected exactly one 品凡小組, found %', matching_teams;
  end if;

  select zones.id into target_zone_id
  from public.zones
  ...
  if target_zone_id is null then
    raise exception 'Could not find 6區 under 神榮耀團隊';
  end if;
  ...
  alter table public.profiles
    drop constraint if exists profiles_team_zone_fkey;

  update public.teams
  set zone_id = target_zone_id,
      sort_order = 6
  where id = target_team_id
    and zone_id is distinct from target_zone_id;

  get diagnostics changed_teams = row_count;
  if changed_teams <> 1 then
    raise exception 'Expected to move exactly one 品凡小組, moved %', changed_teams;
  end if;
  ...
  alter table public.profiles
    add constraint profiles_team_zone_fkey
  ...
  if exists (
    ...
    where teams.id = target_team_id
      and (zones.name <> '6區' or team_groups.name <> '神榮耀團隊')
  ) then
    raise exception '品凡小組 hierarchy validation failed';
  end if;
end;
$$;
...
-- Before the formal activity opens, the only possible rows came through the
-- trusted development override. Classify them as tests so they cannot seed the
-- official W1 cap, map, photo wall, or dashboard.
update public.reports
set is_test = true
where created_at < timestamptz '2026-08-30 16:00:00+00';

comment on column public.reports.is_test is
  '由 trusted server 決定；預上線與本機 override 回報不得計入正式活動。';

alter table public.team_progress
  add column is_test boolean not null default false;

alter table public.team_progress drop constraint team_progress_pkey;
alter table public.team_progress
  add primary key (team_id, activity_week, is_test);

create index reports_team_week_test_status_created_idx
  on public.reports(team_id, activity_week, is_test, status, created_at, id);
...
before insert or update of
  ...
  photo_is_valid, status, created_at, activity_week, is_test
on public.reports
...
begin
  ...
  insert into public.team_progress (
    team_id, activity_week, starts_on, ends_on, accepted_score, is_test, updated_at
  ) values (
    target_team_id,
  ...
  on conflict (team_id, activity_week, is_test) do update
...
begin
  if tg_op = 'DELETE' then
    perform public.recalculate_team_week(old.team_id, old.activity_week, old.is_test);
    return old;
  elsif tg_op = 'INSERT' then
    perform public.recalculate_team_week(new.team_id, new.activity_week, new.is_test);
    return new;
  end if;
...
after insert or delete or update of
  ...
  created_at, activity_week, is_test
on public.reports
...
select
  ...
  team_progress.is_test
from public.team_progress
group by team_progress.team_id, team_progress.is_test;
...
begin
  ...
  if normalized_photo_path is not null then
    if split_part(normalized_photo_path, '/', 1) <> p_reporter_id::text
      or normalized_photo_path !~ ('^' || p_reporter_id::text || '/[0-9a-f]{8}-[0-9a-f]{4}-[4][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}[.](jpg|jpeg|png|webp)$') then
  ...
  insert into public.reports (
    ...
    photo_is_valid, status, activity_week, is_test
  ) values (
  ...
  where team_id = reporter_team_id and is_test = p_is_test;
  ...
  return jsonb_build_object(
    ...
    'is_test', created_report.is_test
  );
exception
...
create function public.submit_report_trusted(
  p_reporter_id uuid,
  p_friend_alias text,
  p_mission_id integer,
  p_is_3x5 boolean,
...
begin
  ...
  select team_id, activity_week, is_test
  into target_team_id, target_week, target_is_test
  from public.reports
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      target_team_id::text || ':' || target_week::text || ':' || target_is_test::text,
      0
    )
  );
  update public.reports
  ...
  perform public.recalculate_team_week(target_team_id, target_week, target_is_test);
  ...
  values (admin_id, 'report_voided', p_report_id,
    jsonb_build_object('reason', btrim(p_reason), 'is_test', target_is_test));
end;
$$;

create function public.admin_dashboard_summary(p_activity_week smallint, p_is_test boolean)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  with filtered_reports as (
    ...
    where status = 'active'
      ...
      and (p_is_test is null or is_test = p_is_test)
  ),
  kpis as (
...
begin
  ...
  with progress as (
    ...
    from public.team_progress
    where p_is_test is null or is_test = p_is_test
...
begin
  ...
  with filtered as (
    ...
    where reports.status::text in ('active', 'void')
      ...
      and (p_is_test is null or reports.is_test = p_is_test)
      and (p_team_group_id is null or team_groups.id = p_team_group_id)
      and (p_zone_id is null or zones.id = p_zone_id)
      and (p_team_id is null or teams.id = p_team_id)
  ...
  select jsonb_build_object(
    ...
    'items', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'is_test', is_test, 'reporter_name', reporter_name,
...
-- Existing rows are official. Rebuild only that scope; test aggregates are
-- created lazily when the first test report arrives.
do $$
